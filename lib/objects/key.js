...
aeq.Key.prototype = {
	...
	setKeyInfo: function ( keyInfo ) {
		keyInfo.value && this.value(keyInfo.value);
		keyInfo.interpolationType && this.interpolationType(keyInfo.interpolationType.inType, keyInfo.interpolationType.outType);

		var interpolationType = this.interpolationType();
		
		// These do not have any effect if interpolationType is not Bezier for in and out
		if (interpolationType.inType === KeyframeInterpolationType.BEZIER && interpolationType.outType === KeyframeInterpolationType.BEZIER) {
			this.temporalAutoBezier(keyInfo.temporalAutoBezier);
			this.temporalContinuous(keyInfo.temporalContinuous);
		}
		
		// TODO: find out why this check is here, was like that in rd_scooter
		if (interpolationType.outType !== KeyframeInterpolationType.HOLD && keyInfo.temporalEase) {
			var inEase = aeq.arrayEx(keyInfo.temporalEase.inEase).map(function (item) {
				return new KeyframeEase(item.speed, item.influence);
			});
			var outEase = aeq.arrayEx(keyInfo.temporalEase.outEase).map(function (item) {
				return new KeyframeEase(item.speed, item.influence);
			});
			this.temporalEase(inEase, outEase);
		}
		
		// These attributes throws an error if valuetype is not spatial when setting
		if (this.valueTypeIs('TwoD_SPATIAL') || this.valueTypeIs('ThreeD_SPATIAL')) {
			keyInfo.spatialAutoBezier && this.spatialAutoBezier(keyInfo.spatialAutoBezier);
			keyInfo.spatialContinuous && this.spatialContinuous(keyInfo.spatialContinuous);
			keyInfo.spatialTangent && this.spatialTangent(keyInfo.spatialTangent);
			keyInfo.roving && this.roving(keyInfo.roving);
		}
	},
